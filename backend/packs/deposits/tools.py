...
def register_python_tools(ctx: PackContext) -> None:
    ...
    ctx.register_python_tool(
        ...
        python_source=(
            ...
            '            return 1.0, "suffix `mm` → already in $MM"\n'
            ...
            '            max_abs = 0.0\n'
            '        if max_abs > 1e7:\n'
            '            return 1e-6, f"max |value| = {max_abs:,.0f} > 1e7 → likely raw $1, ×1e-6 → $MM"\n'
            '        if 0 < max_abs < 1e-3:\n'
            '            return 1000.0, f"max |value| = {max_abs:.5f} < 1e-3 → likely $B, ×1000 → $MM"\n'
            '        return 1.0, f"max |value| = {max_abs:,.2f} → assumed $MM"\n'
            '\n'
            ...
            '        )\n'
            '        # When the balance was detected as non-MM but the metric\n'
            '        # was not (because per-period IE is too small to trip the\n'
            '        # magnitude heuristic on its own), inherit the balance\'s\n'
            '        # scale — files almost always use a consistent unit across\n'
            '        # all monetary columns. The explicit override still wins.\n'
            '        if (\n'
            '            metric_scale_to_mm is None\n'
            '            and bal_scale != 1.0\n'
            '            and met_scale == 1.0\n'
            '            and not str(actual_metric).lower().endswith(("mm", "_mm"))\n'
            '        ):\n'
            '            met_scale = bal_scale\n'
            '            met_scale_note = (\n'
            '                f"inherited from balance ({bal_scale}); IE per-period values "\n'
            '                "too small to detect on their own"\n'
            '            )\n'
            '    else:\n'
