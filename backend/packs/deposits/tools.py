...
def register_python_tools(ctx: PackContext) -> None:
    ...
    ctx.register_python_tool(
        ...
        python_source=(
            ...
            '\n'
            '    # ── Rate scale auto-detect ──────────────────────────────────────\n'
            '    # Rate columns commonly arrive in three flavors:\n'
            '    #   percent  (e.g. 5.25 for 5.25%)        → divide by 100\n'
            '    #   decimal  (e.g. 0.0525 for 5.25%)      → keep as-is\n'
            '    #   bps      (e.g. 525 bps for 5.25%)     → divide by 10,000\n'
            '    # Picking the wrong one shifts the formula by 100× while the\n'
            '    # file\'s pre-computed interest_expense column stays correct,\n'
            '    # which surfaces as a "computed vs data" reconciliation gap of\n'
            '    # exactly 100×. Detection: max |rate| in the file.\n'
            '    rate_series_raw = pd.concat([\n'
            '        m[f"{actual_rate_var_name}_ben"], m[f"{actual_rate_var_name}_cur"]\n'
            '    ], ignore_index=True)\n'
            '    if rate_scale_to_decimal is not None:\n'
            '        rate_div, rate_scale_note = float(rate_scale_to_decimal), f"explicit override (÷{rate_scale_to_decimal})"\n'
            '    else:\n'
            '        try:\n'
            '            rate_max_abs = float(rate_series_raw.abs().max())\n'
            '        except Exception:\n'
            '            rate_max_abs = 0.0\n'
            '        if rate_max_abs > 100:\n'
            '            rate_div, rate_scale_note = 10000.0, f"max |rate| = {rate_max_abs:,.2f} > 100 — likely bps, ÷10,000 → decimal"\n'
            '        elif rate_max_abs > 1:\n'
            '            rate_div, rate_scale_note = 100.0, f"max |rate| = {rate_max_abs:.3f} > 1 — likely percent, ÷100 → decimal"\n'
            '        else:\n'
            '            rate_div, rate_scale_note = 1.0, f"max |rate| = {rate_max_abs:.4f} ≤ 1 — already in decimal, no division"\n'
            '    if rate_div != 100.0:\n'
            '        assumption_notes.append(f"Rate unit detected: {rate_scale_note}")\n'
            '\n'
            '    rate_b = m[f"{actual_rate_var_name}_ben"] / rate_div\n'
            '    rate_s = m[f"{actual_rate_var_name}_cur"] / rate_div\n'
            '\n'
            ...
            '    # in $MM. Used by the waterfall renderer to anchor the chart\n'
            '    # on the baseline IE (left bar) and end at the stress IE\n'
            '    # (right bar). Formula-derived so the chart reconciles with\n'
            '    # the V/M/R decomposition exactly:\n'
            '    #   current_ie_mm = benchmark_ie_mm + total_variance_mm\n'
            '    benchmark_ie = round(float((bal_b * rate_b * period_factor).sum()), 2)\n'
            '    current_ie   = round(float((bal_s * rate_s * period_factor).sum()), 2)\n'
            '\n'
            ...
            '        "rate_divisor_to_decimal":               round(float(rate_div), 4),\n'
            '        "balance_scale_note":                    bal_scale_note,\n'
            '        "metric_scale_note":                     met_scale_note,\n'
            '        "rate_scale_note":                       rate_scale_note,\n'
            '        "aggregation_method":                    (\n'
            ...
            '        "benchmark_ie_mm":            benchmark_ie,\n'
            '        "current_ie_mm":              current_ie,\n'
