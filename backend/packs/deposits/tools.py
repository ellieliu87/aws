...
def register_python_tools(ctx: PackContext) -> None:
    ...
    ctx.register_python_tool(
        ...
        python_source=(
            ...
            '    decomposed_total = round(rate_total + vol_total + mix_total, 2)\n'
            '    # The decomposition is exact when IE = bal * rate * period_factor.\n'
            '    # When the source IE comes from a more complex model output\n'
            '    # (accrual basis, day-count, intra-period averaging), the sum\n'
            '    # of the three effects won\'t exactly equal the actual delta.\n'
            '    # Surface the residual so the waterfall reconciles.\n'
            '    residual = round(round(total_var, 2) - decomposed_total, 2)\n'
            '\n'
            '    return {\n'
            '        "current_scenario":           current_scenario,\n'
            '        "benchmark_scenario":         benchmark_scenario,\n'
