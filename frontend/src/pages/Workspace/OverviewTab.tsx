...
function loadShowInsights(fn: string): boolean {
  try { return localStorage.getItem(showInsightsKey(fn)) === 'true' } catch {}
  return false
}
function saveShowInsights(fn: string, v: boolean) {
  try { localStorage.setItem(showInsightsKey(fn), v ? 'true' : 'false') } catch {}
}
...
export default function OverviewTab({ functionId, onAskAgent, onContextChange }: Props) {
  ...
  useEffect(() => {
    ...
    setShowInsightsCard(loadShowInsights(functionId))
  ...
  useEffect(() => { saveShowInsights(functionId, showInsightsCard) }, [functionId, showInsightsCard])
  ...
  const cards: CardMeta[] = useMemo(() => {
    ...
    // Insights card only appears when the user explicitly adds it via
    // Add Text → Generate Insights. Defaults to hidden for new workspaces
    // so the Overview starts empty rather than auto-generating on every load.
    if (showInsightsCard) {
      out.push({
    ...
    return out
  }, [data, pinnedTiles, textCards, showInsightsCard])
  ...
  const addInsightsCard = () => {
    ...
    setLayout((prev) => {
      const shifted = prev.map((l) => ({ ...l, y: l.y + newH }))
      return [{ i: 'insights', x: 0, y: 0, w: 12, h: newH, minH: 3 }, ...shifted]
    })
    setTimeout(() => window.scrollTo({ top: 0, behavior: 'smooth' }), 50)
  }
  ...
  const removeCard = (cardId: string) => {
    if (cardId.startsWith('text:')) {
      ...
      unpin(plotId)
    } else if (cardId === 'insights') {
      setShowInsightsCard(false)
      setLayout((prev) => prev.filter((l) => l.i !== 'insights'))
    } else {
      hideCard(cardId)
    }
  }
  ...
  return (
    <div>
      ...
      {visibleCards.length === 0 && (
        hiddenCount > 0 ? (
          // All existing cards have been manually hidden
          <div className="panel text-center" style={{ padding: '40px 20px', borderStyle: 'dashed' }}>
            ...
            <div className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>
              Click <strong>Edit Layout</strong> → <strong>Reset Layout</strong> to bring them back.
            </div>
          </div>
        ) : (
          // Fresh workspace — no pinned tiles, no added cards
          <div className="panel text-center" style={{ padding: '48px 24px', borderStyle: 'dashed' }}>
            <Pin size={22} style={{ color: 'var(--text-muted)', margin: '0 auto 10px' }} />
            <div className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>
              This overview is empty
            </div>
            <div className="text-xs mt-2 max-w-xs mx-auto" style={{ color: 'var(--text-muted)', lineHeight: 1.6 }}>
              Go to the <strong>Reporting</strong> tab, build tiles, and click <strong>Pin</strong> on any tile to surface it here.
            </div>
